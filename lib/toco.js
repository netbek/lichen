...
Toco.prototype = {
  ...
  renderHtml: function (markdown) {
    ...
    $('img[data-responsive-image-src]').each(function () {
      ...
      var styles = $elm.data('responsiveImageStyles');
  ...
  loadContent: function (filter) {
    ...
    return this.loadFiles(contentSrc)
      .then(function (files) {
  ...
  buildSitemap: function (filter) {
    var config = this.config;
    var dist = config.env.dev ? config.temp : config.dist;
    var distURL = config.env.dev ? config.tempURL : config.distURL;
    ...
    return this.loadContent(filter)
      .then(function (files) {
        ...
        _.forEach(files, function (file) {
          ...
          var absPath = path.resolve(dist, dirname, basename + '.' + renderExtname);
          var url = distURL + path.relative(path.resolve(dist), absPath);
  ...
  buildContent: function (filter) {
    ...
    return this.loadData()
      ...
      .then(function (files) {
        ...
        _.forEach(files, function (file) {
          _.forEach(file.tasks.mathjax, function (task) {
            tasks.push(task);
          });
        });
        ...
        _.forEach(files, function (file) {
          _.forEach(file.tasks.penrose, function (task) {
            _.forEach(task.styleNames, function (styleName) {
              ...
              var uri = config.env.dev ? penrose.setScheme(task.src, TEMPORARY) : task.src;

              tasks.push({
                style: style,
                src: task.src,
                dist: penrose.getStylePath(styleName, uri)
              });
            });
          });
        });
