...
function Toco(config) {
  ...
  if (_.isUndefined(config.templateDirs)) {
    templateDirs.push(path.resolve(config.src, templateDirname));
  }
  else {
    templateDirs = templateDirs.concat(config.templateDirs);
  }

  templateDirs.push(path.resolve(__dirname, '..', templateDirname));

  var nunjucksEnv = nunjucks.configure(templateDirs);

  _.forEach(config.filters, function (filter, filterName) {
  ...
  md.penrose = new Penrose(config.penrose);
...
Toco.prototype = {
  ...
  findContentTemplate: function (templateName, contentType) {
    ...
    if (_.isUndefined(contentType)) {
      templateDirs = [
        path.resolve(this.config.src, templateDirname) + '/',
        path.resolve(__dirname, '..', templateDirname) + '/'
      ];
    }
    else {
      templateDirs = [
        path.resolve(this.config.src, templateDirname, contentType) + '/',
        path.resolve(__dirname, '..', templateDirname, contentType) + '/'
      ];
    }
  ...
  findPartialTemplate: function (templateName) {
    var templateDirs = [
      path.resolve(this.config.src, templateDirname, partialTemplateDirname) + '/',
      path.resolve(__dirname, '..', templateDirname, partialTemplateDirname) + '/'
    ];
  ...
  build: function () {
    ...
    return this.loadData(dataSrc)
      ...
      .then(function (files) {
        ...
        _.forEach(files, function (file) {
          var templateFile = self.findContentTemplate(file.meta.template, file.meta.type);

          if (_.isUndefined(templateFile)) {
            throw new Error('Content template not found: ' + file.meta.template + ', ' + file.meta.type);
          }
