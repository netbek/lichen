...
function spell(retext, options) {
  ...
  function shouldIgnore(word) {
    return _.includes(ignore, word.toLowerCase()) || (ignoreDigits && /^\d+$/.test(word));
  }
  ...
  return function (tree, file) {
    ...
    if (ignoreCurrency || ignoreEmail || ignoreFqdn || ignoreUrl || ignoreUnits) {
      visit(tree, 'WordNode', function (node, index, parent) {
        ...
        if ((ignoreCurrency && validator.isCurrency(word)) || (ignoreEmail && validator.isEmail(word)) || (ignoreFqdn && validator.isFQDN(word)) || (ignoreUrl && validator.isURL(word)) || (ignoreUnits && unitFilter.test(word))) {
