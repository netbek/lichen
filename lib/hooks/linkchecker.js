    ...
    run: function (html, filePath) {
      return new Promise(function (resolve, reject) {
        ...
        $('a').each(function (i, el) {
          ...
          if (_.isUndefined(scheme) || schemes.indexOf(scheme.toLowerCase()) < 0) {
            return;
          }
        ...
        Promise.mapSeries(chunks, function (chunk) {
            return requestPromise({
              ...
              .then(function (response) {
                ...
                console.log('  ', chalk.green('✓'), chunk.href, '(' + chunk.text + ')', chalk.cyan('(' + title + ')'));

                return Promise.resolve();
              })
              .catch(function (response) {
                console.log('  ', chalk.yellow(response.statusCode), chalk.yellow(chunk.hrefResolved), chunk.href);
