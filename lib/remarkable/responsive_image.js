...
module.exports = function (md) {
  ...
  md.renderer.rules.responsive_image = function (tokens, idx, options, env) {
    var src = (md.config.plugins.responsiveImage.prependSrc ? md.config.plugins.responsiveImage.prependSrc : '') + tokens[idx].src;
    var styleNames = [];
    var sources = [];

    _.forEach(md.config.plugins.responsiveImage.srcset, function (source) {
      ...
      var derivativePath = md.penrose.getDerivativePath(source.style, src, md.penrose.config.srcBase);

      styleNames.push(source.style);
      sources.push(derivativePath + ' ' + source.width + 'w');
    });
    ...
    return nunjucks.render(templateFile, {
      ...
      sizes: md.config.plugins.responsiveImage.sizes,
      srcset: sources.join(', '),
      alt: tokens[idx].alt,
