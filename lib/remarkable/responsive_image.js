...
module.exports = function(md) {
  ...
  function resolveDistPath(styleName, uri) {
    ...
    if (_.isUndefined(scheme)) {
      ...
      return path.join(
        md.config.files.dist[build].path,
        'styles',
        styleName,
        dirname,
        basename + extname
  ...
  md.renderer.rules.responsive_image = function(tokens, idx, options, env) {
    ...
    _.forEach(md.config.remarkable.rulesConfig.responsiveImage.srcset, function(
      source
    ) {
    ...
    return nunjucks.render(templateFile, {
      ...
      sizes: md.config.remarkable.rulesConfig.responsiveImage.sizes,
      srcset: sources.join(', '),
      alt: tokens[idx].alt,
