...
Lichen.prototype = {
  ...
  /**
   *
   * @todo Implement post- (and maybe pre) processing of HTML, and move this code there
   * @param {String} html
   * @return {Promise}
   */
  typesetMath: function (html) {
    return new Promise(function (resolve, reject) {
      mathjax.typeset({
        ...
        if (result.errors) {
          ...
          $('#MathJax_SVG_styles, .MathJax_SVG, .MathJax_SVG_Display').remove();
          $('#MathJax_SVG_glyphs').parent().remove();

          $('script[type^="math/tex"]').each(function (i, elm) {
            var $this = $(this);
            var inline = $this.attr('type').indexOf('mode=display') < 0;
            var alt = $this.text().replace(/\s/g, ' '); // Replace newline and other whitespace with space.

            $this.replaceWith('<img alt="' + _.escape(alt) + '" />');
          });
