...
Lichen.prototype = {
  ...
  renderHtml: function (markdown, filePath) {
    var config = this.config;
    var preHooks = this.loadHooks(PRE_RENDER_HTML, config.env.dev ? DEV : PROD);
    var postHooks = this.loadHooks(POST_RENDER_HTML, config.env.dev ? DEV : PROD);
    var tasks = {
      penrose: []
    };

    // Run pre-Markdown to HTML hooks.
    return this.runHooks(_.clone(markdown), filePath, preHooks)
      .then(function (result) {
        ...
        return this.runHooks(result, filePath, postHooks);
      }.bind(this))
      .then(function (result) {
