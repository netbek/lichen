...
function Lichen(config) {
  ...
  _.forEach(
    globby.sync(['./nunjucks/filters/*.js'], {
      cwd: __dirname
    }),
    function (file) {
  ...
  _.forEach(
    globby.sync(['./nunjucks/globals/*.js'], {
      cwd: __dirname
    }),
    function (file) {
      var name = path.basename(file, path.extname(file));
...
Lichen.prototype = {
  ...
  loadFiles: function (src) {
    return globby(src).then(function (matched) {
      var files = [];
        ...
        return Promise.mapSeries(files, function (file) {
          ...
          if (entryPoints.length) {
            ...
            config.webpack.output = {
              filename: '[name].js',
              path: path.resolve(path.join(config.files.dist[build].path, 'js'))
            };
