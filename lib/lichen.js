...
Lichen.prototype = {
  ...
  loadContent: function (filter) {
    ...
    if (_.isUndefined(config.pages.src.content)) {
      ...
      baseAbs = path.resolve(config.pages.src.path, contentDirname);
    }
    else {
      ...
      if (_.isUndefined(config.pages.src.content.base)) {
        baseAbs = path.resolve(config.pages.src.path, contentDirname);
      }
      else {
        baseAbs = path.resolve(config.pages.src.content.base);
      }
    }
    ...
    return this.loadFiles(src)
      .then(function (files) {
        ...
        return Promise.mapSeries(files, function (file) {
          ...
          var relPath = _.trim(file.path.substring(baseAbs.length), '/');
