...
Lichen.prototype = {
   ...
   * @returns {Object}
   */
  getContentPaths: function (srcPath) {
    var config = this.config;
    ...
    var relPath = _.trim(srcPath.substring(basePath.length), '/');
    var extname = path.extname(relPath);
    var basename = stripModifiers(path.basename(relPath, extname));
    var dirname = path.dirname(relPath);
    var distPath = path.resolve(buildDistPath, dirname, basename + '.' + renderExtname);
  ...
  loadContent: function (filter) {
    ...
    return this.loadFiles(src)
      .then(function (files) {
        ...
        return Promise.mapSeries(files, function (file) {
          _.forEach(this.getContentPaths(file.path), function (value, key) {
            file[key] = value;
          });
