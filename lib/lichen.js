...
function Lichen(config) {
  ...
  _.forEach(multiGlobSync(['./nunjucks/filters/*.js'], {
    cwd: __dirname
  }), function (file) {
  ...
  _.forEach(multiGlobSync(['./nunjucks/globals/*.js'], {
    cwd: __dirname
  }), function (file) {
    var name = path.basename(file, path.extname(file));
    var fn = require(file);
  ...
  // Load default rules.
  _.forEach(multiGlobSync(['./remarkable/*.js'], {
    cwd: __dirname
  }), function (file) {
    md.use(require(file));
  });
      ...
      .then(function (files) {
        ...
        return Promise.mapSeries(files, function (file) {
            return Promise.mapSeries(file.tasks.webpack, function (task) {
              ...
              var jsxPath = path.resolve(config.files.temp.path, 'webpack', basename + '.jsx');
          ...
          .then(function (arr) {
            var entryPoints = _.reduce(arr, function (result, values) {
            ...
            if (entryPoints.length) {
              ...
              config.webpack.plugins = [
                new webpack.optimize.CommonsChunkPlugin({
                  name: 'shared',
                  filename: path.join(config.files.dist[build].path, 'js', 'shared.js'),
                  async: false,
      ...
      .then(function (files) {
        ...
        return Promise.mapSeries(files, function (file) {
          ...
          var context = {
            ...
            page: _.defaults({
              basename: path.basename(file.distPath, path.extname(file.distPath)),
              content: file.html,
              lead: file.leadHtml,
              path: file.distUrl,
              toc: file.tocHtml
